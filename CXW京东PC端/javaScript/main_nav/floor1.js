...
//					//判断鼠标移进去时,是否有li标签变红的,如果没有,用默认样式
				...
				//所有楼层通用
				//移动进内容里面上面的标签还是高亮
				$(".floor .main").mouseenter(function(){
					var index = $(this).index()-1;
					//移动进内容里面上面的标签还是高亮
					$("#floor_1 .tab li").removeClass("tab-selected");
					$("#floor_1 .tab li").eq(index).addClass("tab-selected");
					$("#floor_2 .tab li").removeClass("tab-selected");
					$("#floor_2 .tab li").eq(index).addClass("tab-selected");
					$("#floor_3 .tab li").removeClass("tab-selected");
					$("#floor_3 .tab li").eq(index).addClass("tab-selected");
					//当滑到对应的下标时,对应的num出现
					//通过改变num(也就是main)的class来实现
//					//判断鼠标移进去时,是否有li标签变红的,如果没有,用默认样式
				//当鼠标划出地址的时候下拉框消失
